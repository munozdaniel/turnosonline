...
class SolicitudturnoController extends ControllerBase
{
    ...
    private function seleccionarTemplateCodigo($mensaje)
    {
        $template = file_get_contents('http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['SCRIPT_NAME']) . '/files/emailtemplate/codigo.volt');
     ...
     */
    public function aceptaAsistenciaTablaAjaxAction()
    {
        $this->view->disable();
        $retorno = array();
        $retorno['success'] = false;
    ...
    /**
     * Cancela la asistencia, se utiliza desde turnos respondidos, en el modal.
     */
    public function cancelaAsistenciaTablaAjaxAction()
    {
        $this->view->disable();
        $retorno = array();
        $retorno['success'] = false;
